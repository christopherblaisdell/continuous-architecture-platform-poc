...
| [Customization Portability: Option D + OpenSpec](customization-lock-in-foundry-vs-portable.md) | This page establishes that customizations should be practitioner-controlled living artifacts. The portability page shows how that same living practice model also serves as a lock-in remediation — customizations that architects maintain in portable files cannot be locked into any single platform. OpenSpec adds cross-platform generation for 25+ tools. |
