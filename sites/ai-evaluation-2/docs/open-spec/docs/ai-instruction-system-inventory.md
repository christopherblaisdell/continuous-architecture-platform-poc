# AI Instruction System — Complete Inventory

This document captures the full state of the AI instruction governance system
...
The system enforces a hub-and-spoke architecture for AI instruction rules:

- **Canonical hub**: `.ai-instructions/` — the only place rules are authored
...
- **Validation**: `scripts/validate-ai-instructions.sh` — 32 checks, runs on pre-commit
- **Governance spec**: `openspec/specs/ai-instruction-governance/spec.md`

---

## File Inventory

### Canonical Hub — `.ai-instructions/`
...
#### `.ai-instructions/universal/` — Always loaded for all modes
...
#### `.ai-instructions/methodologies/` — Workflow instructions
...
#### `.ai-instructions/standards/` — Domain standards
...
#### `.ai-instructions/mode-customizations/` — Per-mode instructions
...
#### `.ai-instructions/user-prompts/` — VS Code user-level prompts
...
**Note:** `sync-ai-instructions.sh` is the script that regenerates these files. It does not yet exist (Phase 4 was skipped). Regeneration is currently done manually by the AI agent during apply.
...
| `openspec/specs/ai-instruction-governance/spec.md` | Governance spec — REQ-GOV-001 through REQ-GOV-004 |
| `openspec/schemas/ai-instruction-change/schema.yaml` | Custom schema forked from `spec-driven` |
| `openspec/schemas/ai-instruction-change/templates/proposal.md` | Proposal template — canonical/derived file inventory + governance checklist |
| `openspec/schemas/ai-instruction-change/templates/tasks.md` | Tasks template — edit canonical → sync → verify → commit |
| `openspec/schemas/ai-instruction-change/templates/design.md` | Design template (from spec-driven fork, not customized) |
| `openspec/schemas/ai-instruction-change/templates/spec.md` | Spec template (from spec-driven fork, not customized) |
...
rules:
  ...
  tasks:
    - Always include a task to run scripts/sync-ai-instructions.sh
    - Always include a task to verify scripts/validate-ai-instructions.sh passes
```
...
| File | Purpose | Status |
|---|---|---|
| `scripts/validate-ai-instructions.sh` | 32-check validation: file existence, YAML frontmatter, paired sync, key rules, derived headers, OpenSpec checks | Active — runs on pre-commit |
| `scripts/sync-ai-instructions.sh` | Regenerates all 5 derived files from canonical hub atomically | **NOT YET CREATED** (Phase 4 skipped) |
| `scripts/check-conversation-size.sh` | Conversation token size utility | Unrelated to AI instructions |
| `scripts/clone-external-repos.sh` | External repo cloning | Unrelated to AI instructions |
| `scripts/export-conversation-context.sh` | Conversation context export | Unrelated to AI instructions |
...
| Hook trigger | Runs `validate-ai-instructions.sh` when AI instruction files are staged | Working |
| `.gitignore` | Explicitly un-ignores `.ai-instructions/**`, `openspec/**`, `.clinerules` | Configured |
...
3. If any match, runs `scripts/validate-ai-instructions.sh`
...
| File | Purpose |
|---|---|
| `plans/openspec-ai-instruction-governance-plan.md` | 11-phase integration plan — the master plan for this governance system |
...
| Phase 4 | Create `scripts/sync-ai-instructions.sh` | **Skipped** (defer to team rollout) |
...
| Phase 7 | Create `openspec/schemas/ai-instruction-change/` custom schema | Complete (`169bdc7`) |
| Phase 8 | Add "How to Make Changes" section to `.ai-instructions/README.md` | Complete (`1b1a3ca`) |
...
### `sync-ai-instructions.sh` — Not Yet Created
...
The `scripts/validate-ai-instructions.sh` runs 32 checks in 7 sections:
...
Roo Code loads AI instructions via symlinks created by `setup-ai-tools.sh`:
