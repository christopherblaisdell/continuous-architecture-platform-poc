...
- **Cross-tool accessibility** — fair point. Platform-native indexes are primarily optimized for the IDE and CLI. Microsoft 365 connectors do offer a bridge for surfacing this knowledge in Teams or SharePoint, but the underlying vector data isn't accessible for custom external apps. We've documented this as an acceptable trade-off for now and explore MCP as a bridge for future cross-tool use cases. See the cross-tool section in [Build vs Leverage](https://nbcu-ot.atlassian.net/wiki/spaces/UPA/pages/2615148826/Build+vs+Leverage+Custom+RAG+in+Context)
