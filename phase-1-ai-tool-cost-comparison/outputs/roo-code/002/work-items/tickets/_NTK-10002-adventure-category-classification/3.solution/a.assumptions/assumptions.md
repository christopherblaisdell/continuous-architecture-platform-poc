...
The set of 25 adventure categories is expected to remain relatively stable. New categories may be added at most once or twice per quarter. The classification system is designed for low-frequency updates, not continuous change. If category additions become frequent (more than once per sprint), the configuration-driven approach should be reevaluated in favor of a database-backed solution per ADR-NTK10002-001.
...
## A7: Spring Cloud Config Infrastructure Available

The NovaTrek platform already has Spring Cloud Config infrastructure deployed and operational. No additional infrastructure provisioning is required for the classification configuration. The `svc-check-in` service already connects to Spring Cloud Config for existing configuration properties.

## A8: Activity Type Naming Reconciliation

The svc-trip-catalog `ActivityType` enum uses names that differ from the ticket classification table (e.g., `ROCK_CLIMBING` vs `CLIMBING`, `BACKCOUNTRY_SKIING` vs `SKIING`, `WILDLIFE_SAFARI` vs `SAFARI`). The `adventure_category` field is an independent, more granular field that does not require renaming the existing `activity_type` values. Both fields coexist on the Trip schema.
