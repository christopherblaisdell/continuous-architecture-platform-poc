...
|--------------------|--------------|------------------------------------------------------------|
| svc-check-in       | PRIMARY      | New ClassificationService, modified check-in flow, new config, API response addition |
...
Three architecture decision records govern this solution:

1. **ADR-NTK10002-001**: Configuration-driven classification over database-driven -- the category-to-pattern mapping is maintained as YAML in Spring Cloud Config rather than a database table.
2. **ADR-NTK10002-002**: Pattern 3 (Full Service) as the default fallback for unknown, null, or empty categories -- safety-first principle.
3. **ADR-NTK10002-003**: Additive API contract changes -- `determined_pattern` and `pattern_name` added to the CheckIn response, `adventure_category` added to the Trip schema in svc-trip-catalog.

Full MADR-formatted records are in [decisions.md](3.solution/d.decisions/decisions.md).
...
Analysis of the current `AdventureCategoryClassifier.java` in `svc-check-in` reveals critical gaps that this solution addresses:
...
| Unsafe default | Unknown categories default to `PATTERN_1` (Basic) at line 74 of `AdventureCategoryClassifier.java` | Unknown categories MUST default to `PATTERN_3` (Full Service) per ADR-NTK10002-002 |
| No caching | No caching mechanism; categories are read from in-memory constants on every call | Spring `@Cacheable` with configurable TTL for config-driven lookups |
...
## 10. Service Interaction Changes
...
The `Trip` schema in `svc-trip-catalog` must be extended with an `adventure_category` field (string enum, nullable). This field is populated when trips are created or updated and flows through `svc-reservations` to `svc-check-in` at check-in time. The current Trip schema in `svc-trip-catalog.yaml` does not include this field -- it needs to be added.
...
- **Performance tests**: Validate that classification lookup adds negligible latency to check-in request.
