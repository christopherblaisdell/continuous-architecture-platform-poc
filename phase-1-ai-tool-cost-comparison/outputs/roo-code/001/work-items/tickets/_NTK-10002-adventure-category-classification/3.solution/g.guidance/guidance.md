...
Create `ClassificationService.java` in `svc-check-in` to replace the current `AdventureCategoryClassifier.java`:
...
public class ClassificationService {
    ...
    public int determinePattern(String bookingSource, String adventureCategory) {
        ...
        // 3. Fallback to default (Pattern 3 per ADR-NTK10002-002)
        return defaultPattern;
    }
}
```

IMPORTANT: The current `AdventureCategoryClassifier.java` defaults to `PATTERN_1` for unknown categories (line 74). The new `ClassificationService` MUST default to Pattern 3. This is a safety-critical change documented in ADR-NTK10002-002.

## 3. Deprecating AdventureCategoryClassifier

The existing `AdventureCategoryClassifier.java` should be deprecated but NOT removed initially. The feature flag `novatrek.checkin.use-category-classification` controls which classifier is active:

- When `false` (default during rollout): Existing `AdventureCategoryClassifier` is used
- When `true`: New `ClassificationService` is used

After the new classifier is validated in production, the old `AdventureCategoryClassifier` should be removed in a follow-up cleanup ticket.
...
## 5. Order of Operations in Check-in Flow
...
## 6. Updating svc-check-in API Response
...
public class CheckInResponse {
    // existing fields...
    private Integer determinedPattern;  // 1, 2, or 3
