...
## A7: Existing AdventureCategoryClassifier Will Be Replaced

The current `AdventureCategoryClassifier.java` uses hardcoded category sets that do not match the approved 25-category list. This class will be fully replaced by the new `ClassificationService`, not extended. The feature flag controls which implementation is active during the transition period.

## A8: svc-reservations Passes adventure_category Through

The `svc-reservations` service already returns trip metadata including `activity_type`. We assume it can be extended to also return `adventure_category` from `svc-trip-catalog` without significant effort. This is an additive field on the reservation response.
