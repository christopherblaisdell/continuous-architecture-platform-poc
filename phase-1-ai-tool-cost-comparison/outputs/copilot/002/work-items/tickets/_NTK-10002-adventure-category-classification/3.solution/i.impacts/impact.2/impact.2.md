...
| Activity Type | Available Adventure Categories |
|---------------|-------------------------------|
...
The Trip API Swagger specification (`svc-trip-catalog.yaml`) must be updated to include the `adventure_category` field in the Trip response model. The field should be documented with the full list of valid enum values and a description of its purpose.
...
The `adventure_category` field is nullable. Existing API consumers that do not read this field will continue to function unaffected. The existing `activity_type` field is retained and remains the primary activity classifier for all non-check-in use cases.
