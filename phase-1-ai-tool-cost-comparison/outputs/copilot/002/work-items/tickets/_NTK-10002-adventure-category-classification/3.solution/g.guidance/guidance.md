...
Key differences from the current `AdventureCategoryClassifier`:
- Reads from Spring Cloud Config YAML, not hardcoded `Set` constants
- Defaults to Pattern 3 (Full Service) for unknown categories, not Pattern 1
- Uses the approved 25-category list, not the mismatched categories in the current code
- Supports `@Cacheable` for performance
...
The feature flag `novatrek.checkin.use-category-classification` controls which classification logic is active:

- **true** (default after deployment): Use the new `ClassificationService` with config-driven 25-category classification
- **false**: Fall back to the legacy `AdventureCategoryClassifier` with `activity_type`-based two-tier mapping

This enables instant rollback without a deployment if issues are discovered.

## 6. API Response Enhancement

Add two fields to the `CheckIn` response schema:

```yaml
determined_pattern:
  type: integer
  enum: [1, 2, 3]
  description: The check-in UI pattern determined by the classification service
  example: 2
pattern_name:
  type: string
...
Add the following metrics and alerts:

| Metric | Type | Tags | Alert Threshold |
|--------|------|------|-----------------|
| `checkin.pattern.determined` | Counter | pattern={1,2,3} | N/A (informational) |
| `checkin.classification.fallback.count` | Counter | None | Alert when >5% of total classifications |
| `checkin.classification.override.count` | Counter | booking_source | N/A (informational) |
| `checkin.classification.latency` | Timer | None | Alert when p99 >5ms |
