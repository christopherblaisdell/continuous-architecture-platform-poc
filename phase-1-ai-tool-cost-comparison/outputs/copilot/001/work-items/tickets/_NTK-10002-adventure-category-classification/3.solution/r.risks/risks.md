...
## RISK-005: Legacy Data Migration Incompleteness

**Likelihood**: Medium
**Impact**: Low

**Description**: Existing trips in the catalog may not have an `adventure_category` assigned during the initial data migration. If the migration is incomplete, reservations for these trips will have a null `adventure_category`, triggering the Pattern 3 fallback for every check-in.

**Mitigation**: Run the migration with automated category assignment based on `activity_type` and trip metadata. Flag trips that cannot be auto-classified for manual review. Monitor the fallback metric post-deployment to identify gaps. The safety-first fallback ensures no guest is under-served during the transition.
