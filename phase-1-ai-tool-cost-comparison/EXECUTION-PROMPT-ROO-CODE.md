...
2. **Commit results**: `git add -A && git commit -m 'feat: complete Phase 1 Roo Code execution - all 5 scenarios'`
...
4. **Create `phase-1-roo-code-results.md`** documenting scores, observable metrics, and notes.
5. **Run data isolation audit**: `../../scripts/audit-data-isolation.sh`
6. **Reset workspace** if additional runs are needed (re-run the reset commands above).
