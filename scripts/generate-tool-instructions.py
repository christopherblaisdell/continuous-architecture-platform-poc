...
Usage:
  ...
  python3 scripts/generate-tool-instructions.py --tool foundry # write Foundry output only
"""
...
def generate_foundry(dry_run: bool, check: bool) -> list:
    """Generate Azure AI Foundry-compatible output to .foundry/."""
    results = []

    # 1. system-prompt.md — core instructions as a standalone file for hosted agents
    instructions_content = read_source(".openspec/instructions/core-instructions.md")
    system_prompt_header = (
        "# NovaTrek Architecture Platform — System Prompt\n\n"
        "Auto-generated. Do not edit manually.\n"
        "Source: .openspec/instructions/core-instructions.md\n"
        "Regenerate: python3 scripts/generate-tool-instructions.py --tool foundry\n\n"
        "---\n\n"
    )
    results.append(write_or_check(
        ROOT / FOUNDRY_DIR / "system-prompt.md",
        system_prompt_header + instructions_content,
        dry_run, check
    ))

    # 2. agent.yaml — prompt agent deployment config with system prompt inlined
    # Suitable for deploying via the Foundry MCP agent_update tool or az ai foundry CLI.
    instructions_indented = "\n".join(
        "  " + line if line.strip() else ""
        for line in instructions_content.splitlines()
    )
    agent_yaml_content = (
        "# Auto-generated by scripts/generate-tool-instructions.py\n"
        "# Source: .openspec/\n"
        "# Do not edit manually — regenerate with:\n"
        "#   python3 scripts/generate-tool-instructions.py --tool foundry\n"
        "#\n"
        "# Deploy via Foundry MCP tool (agent_update) or:\n"
        f"#   az ai foundry agent update --definition {FOUNDRY_DIR}/agent.yaml\n"
        "\n"
        "type: prompt\n"
        "model: gpt-4o\n"
        "instructions: |\n"
    ...
    results.append(write_or_check(
        ROOT / FOUNDRY_DIR / "agent.yaml",
        agent_yaml_content,
        dry_run, check
    ))

    # 3. agent-metadata.yaml — deployment config stub (operator fills in endpoints + ACR)
    metadata_content = (
        "# Auto-generated stub by scripts/generate-tool-instructions.py\n"
        "# Fill in environment-specific values before deploying.\n"
        "# See: ai-platform-selection/guides/AZURE-AI-FOUNDRY-CUSTOMIZATION-GUIDE.md\n"
        "\n"
        "defaultEnvironment: dev\n"
        "environments:\n"
        "  dev:\n"
        "    projectEndpoint: \"\"  # set to your Foundry project endpoint\n"
        "    agentName: novatrek-solution-architect-dev\n"
        "    azureContainerRegistry: \"\"  # set to your ACR registry\n"
        "  prod:\n"
        "    projectEndpoint: \"\"  # set to your Foundry project endpoint\n"
    ...
    results.append(write_or_check(
        ROOT / FOUNDRY_DIR / "agent-metadata.yaml",
        metadata_content,
        dry_run, check
    ))

    # 4. prompts/ — copy .openspec/prompts/*.prompt.md to .foundry/prompts/<name>.md
    # Strips the .prompt infix from filenames so they are plain markdown files
    # suitable for upload to Foundry Prompt Management or bundling into a hosted agent.
    prompts_src = ROOT / ".openspec" / "prompts"
    for prompt_file in sorted(prompts_src.glob("*.prompt.md")):
        dest_name = prompt_file.name.replace(".prompt.md", ".md")
        content = strip_frontmatter(prompt_file.read_text(encoding="utf-8"))
        results.append(write_or_check(
            ROOT / FOUNDRY_DIR / "prompts" / dest_name,
            content,
            dry_run, check
        ))
...
def main():
    ...
    parser.add_argument("--tool", choices=["cursor", "roocode", "windsurf", "claude", "gemini", "copilot", "foundry"], help="Generate for one tool only")
