...
**Why it matters**: MkDocs, pymdownx, and other build-time dependencies may contain vulnerabilities. Even though these packages only run at build time (not in production), a compromised build dependency could inject malicious content into the generated HTML. This addresses [OWASP CICD-SEC-3 (Dependency Chain Abuse)](https://owasp.org/www-project-top-10-ci-cd-security-risks/).

**Implementation** (`snyk-security.yml`):
...
- name: Run Snyk code analysis
  ...
  with:
    command: code test
    args: --severity-threshold=high portal/scripts/
```
...
- `.github/workflows/*.yml` — overly broad workflow permissions, unpinned action versions
...
```yaml
- name: Run Snyk IaC scan on staticwebapp.config.json
...
| Snyk dependency scan | Automated, blocks merge | Not available |
| Snyk code analysis | Automated, blocks merge | Not available |
...
Beyond the CI gates described above, Snyk's GitHub integration provides continuous monitoring between releases:
...
**GitHub Secret Required**: All three Snyk gates (Gates 4, 5, and 6) require `SNYK_TOKEN` to be configured as a GitHub Actions secret. Without this secret, the scans will fail and the PR cannot be merged. To obtain a token, create a free account at [snyk.io](https://snyk.io) and generate a service account token in your organization settings.
