...
- Given I enter incorrect information, when I submit the form, then I see a generic error message ("We couldn't find a matching reservation") and the number of remaining attempts
...
- Given my session expires before I complete check-in, when I try to continue, then I am returned to the welcome screen and can re-enter my details to start a new session
...
- Given I used my partner confirmation code with a partner-specific prefix (like EM- for ExploreMore), when I enter it at the kiosk, then the system correctly matches my reservation regardless of whether I include the prefix
...
- Given I return for another adventure within 90 days, when I check in at the kiosk again, then I am recognized by my previous temporary profile and offered an easy path to convert to a full account
...
- Given a kiosk has received 5 failed lookup attempts within 15 minutes, when a further attempt is made, then the kiosk displays a cooldown message directing the guest to the service desk for assistance
...
- Given a guest has difficulty verifying, when they reach the attempt limit, then they see the service desk location and are assured they will receive priority assistance
- Given any lookup activity occurs, when reviewed by operations, then all attempts are traceable for security audit purposes with a correlation ID
...
## US-6: Companion Guest Check-in

**As a** companion on someone else's NovaTrek reservation,
**I want to** check in at the kiosk using the reservation confirmation code shared with me,
**So that** I do not need my own NovaTrek account or a separate reservation to participate.

### Acceptance Criteria

- Given I am listed as a participant on a reservation but do not have my own NovaTrek account, when I enter the group's confirmation code and my last name at the kiosk, then I am matched to the reservation and can complete check-in independently
- Given I complete check-in, when I view my session summary, then I see the adventure details and gear assignments for my participation
- Given the primary guest has already checked in, when I check in separately, then the system shows the group's overall check-in progress
