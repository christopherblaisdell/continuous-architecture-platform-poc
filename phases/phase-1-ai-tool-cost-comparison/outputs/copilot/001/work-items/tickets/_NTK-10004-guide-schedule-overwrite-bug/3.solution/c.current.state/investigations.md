...
**Key observations**:
- Three distinct guides affected in a single day
- Guide G-4821 had two PUTs within a 47ms window, confirming concurrent write race condition
- Data lost includes safety-critical certifications (ROPE_COURSE, ZIPLINE_ADVANCED) and qualifications (FIRST_AID_CERTIFIED)

### 1.2 Causal Chain

The ERROR logs directly correlate with downstream operational impact:
- Guide G-4821 lost ROPE_COURSE certification at 08:14 -- would be removed from rope course assignments
- Guide G-5190 lost FIRST_AID_CERTIFIED specialization at 09:15 -- would be removed from trips requiring first aid capability
- Guide G-3302 lost SPANISH, PORTUGUESE language skills at 13:05 -- would be removed from bilingual tour assignments
...
The entity has two fields explicitly vulnerable to overwriting:
...
The controller only exposes PUT -- no PATCH endpoint exists:
...
There is no merge logic, no field-level ownership check, and no concurrency control (no `@Version`, no `If-Match` ETag support).
...
When multiple optimization runs execute concurrently, guides assigned to cross-region trails have their schedules overwritten by the last writer. Elastic logs confirm a 47ms race window for guide G-4821.
