...
The existing `AdventureCategoryClassifier.java` must be replaced. Current issues identified in source code analysis:

| Issue | Line | Current Behavior | Required Behavior |
|-------|------|-----------------|-------------------|
| Hardcoded categories | Lines 13-30 | Static `Set.of(...)` constants | Config-driven YAML mapping |
| Mismatched category names | Lines 13-30 | Uses `SCENIC_OVERLOOK`, `BIRD_WATCHING`, etc. | Must use approved 25-category enum (e.g., `BIRDING_SAFARI`) |
| Unsafe null default | Line 68 | `null` category returns `PATTERN_1` | Must return `PATTERN_3` (Full Service) |
| Unsafe unknown default | Line 78 | Unknown categories return `PATTERN_1` | Must return `PATTERN_3` (Full Service) |
