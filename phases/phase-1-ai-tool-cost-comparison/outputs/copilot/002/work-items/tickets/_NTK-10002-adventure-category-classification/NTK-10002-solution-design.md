...
- **Safety-first defaults**: Unknown or unmapped categories default to Pattern 3 (Full Service). This is a CRITICAL safety requirement — see ADR-NTK10002-002.
...
Three architecture decisions have been made for this ticket. See `3.solution/d.decisions/decisions.md` for full MADR-formatted records:

- **ADR-NTK10002-001**: Configuration-driven classification over database-driven — Accepted
- **ADR-NTK10002-002**: Pattern 3 as default fallback for unknown categories — Accepted
- **ADR-NTK10002-003**: Additive API contract changes for classification response — Accepted
...
| Unsafe default for null | Unknown categories default to `PATTERN_1` (Basic) — line 68 of `AdventureCategoryClassifier.java` | Unknown categories MUST default to `PATTERN_3` (Full Service) per ADR-NTK10002-002 |
| Unsafe default for unknown | Unrecognized categories fall back to `PATTERN_1` — line 78 | Must fall back to `PATTERN_3` (Full Service) |
...
## 13 Quality Attributes (ISO 25010)

| Attribute | Assessment |
|-----------|------------|
| Functional Suitability | Meets all stated requirements: 25-category classification, 3 patterns, booking source overrides, and safe defaults |
| Performance Efficiency | In-memory cache with configurable TTL ensures sub-millisecond classification lookup |
| Compatibility | Backward-compatible: additive fields on existing schemas, feature flag for gradual rollout |
| Reliability | Safety-first fallback to Pattern 3 prevents under-servicing; feature flag enables instant rollback |
| Maintainability | Configuration-driven approach separates business rules from code; YAML config is auditable via Git |
| Security | No PII impact; no new authentication or authorization requirements |
...
| 1.8     | 2026-03-04 | Enhanced with ISO 25010 quality attributes, refined source code analysis |
