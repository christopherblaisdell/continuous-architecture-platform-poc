...
## RISK-005 Legacy Data Migration Gaps

**Likelihood**: Medium
**Impact**: Low

**Description**: Existing trips in `svc-trip-catalog` do not have the `adventure_category` field populated. The data migration to assign categories to existing trips may miss edge cases or misclassify trips based on limited metadata. These trips will fall back to Pattern 3 until manually corrected.

**Mitigation**: The migration should be conservative — flag any trip that cannot be confidently classified for manual review. The safety-first fallback to Pattern 3 ensures no guest is under-serviced during the transition. Monitor the `checkin.classification.fallback.count` metric after migration to identify remaining gaps.
