...
- Given my partner booking has not been synced, when I enter my partner confirmation code at the kiosk, then the system automatically checks with the booking partner for verification
...
- Given my partner booking has already been synced, when I enter my details, then the direct lookup succeeds without requiring the partner fallback
...
- Given I received a reservation confirmation code via email, when I enter my details at the kiosk, then the system finds my reservation and creates a temporary guest profile
- Given I completed check-in with a temporary profile, when I finish my adventure, then I am prompted to create a full NovaTrek account to preserve my history and earn rewards
...
- Given a kiosk has received 5 failed attempts in 15 minutes, when a further attempt is made, then the kiosk displays a rate-limit message directing the guest to the service desk
- Given a lookup attempt fails, when the response is returned, then there is a brief delay before the failure is displayed
- Given any lookup activity occurs, when reviewed by operations, then all attempts are traceable for security audit
...
**So that** I can measure the impact of the self-service kiosk and optimize staffing levels.

### Acceptance Criteria

- Given check-ins have been processed, when I view the analytics dashboard, then I see separate counts for registered-guest kiosk, unregistered-guest kiosk, and staff-assisted check-ins
- Given the analytics data is available, when I filter by date range and base camp, then I see average check-in times and completion rates per channel
- Given a guest abandons the kiosk check-in flow, when I view the dashboard, then I see the abandonment recorded with the exit step
