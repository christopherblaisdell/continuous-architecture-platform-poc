# Phase 1 Execution Prompt: GitHub Copilot (Agent Mode)

> **Purpose**: Self-contained prompt for executing all 5 Phase 1 architecture scenarios in a single, fresh Copilot Agent Mode session. Paste the prompt section into a brand-new Copilot chat with zero prior context.
...
+-- cross-platform-comparison.md                (Comparison with latest Roo Code run)
```
...
After creating `run-summary.md`, generate a comparison report against the latest Roo Code run.

### Steps

1. Find the latest Roo Code run folder:
   ```bash
   ls -d phase-1-ai-tool-cost-comparison/outputs/roo-code/[0-9][0-9][0-9] 2>/dev/null | tail -1
   ```
2. If no Roo Code run exists, create `cross-platform-comparison.md` in your run folder with a note that no Roo Code run is available for comparison yet, then skip to END PROMPT.
3. If a run exists, read these files from the latest Roo Code run folder (some may not exist yet — note which are missing):
   - `run-summary.md` (AI-generated: model turns, files, tool calls)
   - `run-metadata.md` (human-created: wall-clock time, OpenRouter cost, generation IDs)
   - `results.md` (human-created: quality scores per scenario)
4. Read your own `run-summary.md` that you just created.
5. Create `cross-platform-comparison.md` in your run folder (`outputs/copilot/<RUN_NUMBER>/cross-platform-comparison.md`) with:

### Report Structure

```markdown
# Cross-Platform Comparison: Copilot Run <RUN> vs Roo Code Run <RUN>

## Execution Metrics
| Metric | Copilot (this run) | Roo Code (run <N>) |
|--------|-------------------|-------------------|
| Model turns | ... | ... |
...
|--------|---------|----------|
| Cost model | turns x $0.028 x 3 | OpenRouter exact |
| Session cost | $X.XX (calculated) | $X.XX or pending |
| Cost per model turn | $0.084 | $X.XX or pending |
...
- Your own cost is always calculable: model_turns x $0.028 x 3.
- The Roo Code cost comes from their `run-metadata.md` (OpenRouter exact cost). If that file is missing, mark as "pending".
